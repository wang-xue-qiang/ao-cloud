 ...
 */
@Component
public class EquipFallback implements EquipFeignClient {
	
	@Override
	public RespondResult findByIdEquip(String id) {
		return RespondResult.build(500, "调用失败，进行回调",new Address("-1","默认数据"));
	}

	@Override
	public RespondResult pageEquipList(Map<String, Object> params) {
	...
	@Override
	public RespondResult saveEquip(Equip equip) {
		return RespondResult.build(500, "调用失败，进行回调");
	}

	@Override
	public RespondResult deleteEquip(String[] ids) {
		return RespondResult.build(500, "调用失败，进行回调");
	}
